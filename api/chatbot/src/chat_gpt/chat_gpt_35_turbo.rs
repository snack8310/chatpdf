...
use super::{ChatApi, Conversation};
...
impl ChatApi for ChatGpt35Turbo {
    async fn send_message(
        ...
        let messages = create_full_message(prompt, context, message_from_user);
...
fn create_full_message(prompt:String, context: &Vec<Conversation>, message_from_user: &str) -> Vec<Message> {
    let system = get_system_messages(prompt);
...
fn get_system_messages(prompt:String) -> Vec<Message> {
    vec![Message {
